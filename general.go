...
func GeneralHandler(w http.ResponseWriter, r *http.Request, grpcConn *grpc.ClientConn, blockTime uint64) {
	...
	paramsSymbolsCountGauge.Set(float64(len(oracleParamsResponse.Params.Whitelist)))
	...
	baseClient := tmservice.NewServiceClient(grpcConn)
	baseResponse, err := baseClient.GetLatestBlock(
		context.Background(),
		&tmservice.GetLatestBlockRequest{},
	)

	windowProgress := (uint64(baseResponse.GetBlock().GetHeader().Height) %
		oracleParamsResponse.Params.SlashWindow) / oracleParamsResponse.Params.VotePeriod
	...
	go func() {
		...
		missCounterResponse, err := oracleClient.MissCounter(
			context.Background(),
			&oracletypes.QueryMissCounterRequest{ValidatorAddr: myAddress.String()},
		)
		if err != nil {
			sublogger.Error().
				Str("valoper", valoper).
		...
		missRate := float64(missCounterResponse.MissCounter) / float64(windowProgress)
		...
		seconds := (windowSize - windowProgress + 1) * blockTime * oracleParams.VotePeriod
	...
	go func() {
		...
		response, err := oracleClient.FeederDelegation(
			context.Background(),
			&oracletypes.QueryFeederDelegationRequest{ValidatorAddr: myAddress.String()},
		)
		if err != nil {
			sublogger.Error().
				Str("valoper", valoper).
	...
	go func() {
		...
		response, err := oracleClient.AggregatePrevote(
			context.Background(),
			&oracletypes.QueryAggregatePrevoteRequest{ValidatorAddr: myAddress.String()},
		)
		if err != nil {
			sublogger.Warn().
				Str("valoper", valoper).
	...
	go func() {
		...
		response, err := oracleClient.AggregateVote(
			context.Background(),
			&oracletypes.QueryAggregateVoteRequest{ValidatorAddr: myAddress.String()},
		)
		if err != nil {
			sublogger.Warn().
				Str("valoper", valoper).
